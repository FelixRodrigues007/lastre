# Super Prompt for Laura — Lastre Frontend Design

Copy/paste this into Claude, Cursor, or the design/build tool Laura is using.
...
- Do not implement trust logic in the frontend. The gateway returns the live state.
- The deterministic SHA-256 seal decides Valid/Invalid. The UI only presents it.
...
ownership, or transferable token rights. The user action is always Verify provenance
or Spot the fraud.

Use fictional data only. If a lot has no on-chain attestation, label it Unverified
or Simulated. Never fake a live verdict.
...
- no generic crypto purple/cyan gradients;
- no AI magic visuals;
- no trading dashboard language.
...
POST /sandbox/anchor (controlled SANDBOX-only; hidden by default)
GET /fraud-challenge?assetId=...&difficulty=easy|hard
POST /fraud/guess
POST /fraud/anchor-tampered (controlled SANDBOX-only; hidden by default)
...
- Show Correct/Wrong feedback, streak, and score
- Optional Anchor tampered seal button only after reveal and only in SANDBOX mode
- Respect prefers-reduced-motion

COPY STYLE
Short, technical, confident.
Use words like proof, seal, origin, provenance, on-chain, verified, recorded.
Avoid vague phrases like revolutionary, next-gen, magical, unlock value, democratize access.
...
A tiny tamper changes the seal. The seal decides the verdict. The chain records both Valid and Invalid. Lastre proves provenance before any token or agent acts.
```
